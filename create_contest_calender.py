...
for tstr, dstr, name in zip(df[1]['Start Time (local time)'],df[1]['Duration'], df[1]['Contest Name']):
    ...
    name = name.replace('AtCoder Beginner Contest','ABC')
    name = name.replace('AtCoder Regular Contest','ARC')
    name = name.replace('AtCoder Heuristic Contest','AHC')
    name = name.replace('Programming Contest','PC')
    name = name.replace(year,'')
    name = name.replace('◉  ','')
    name = name.replace(' ','')
    ContestName.append(name)
